...
async function main() {
  ...
  const abstract = createEditorState(
  ...
  await prisma.threadMessage.create({
    data: {
      ...
      body: createEditorState(
        `Author responses and other discussion on public reviews can be viewed in a thread below the review.`
      ),
      highlights: "[]",
    },
  });
  ...
  const decision = await prisma.decision.create({
    data: {
      authorId: reviewer.id,
      body: createEditorState(
        `This is an example meta-review. Reviews that an author cites in writing the meta-review are attached to the meta-review, and directly credit the reviewer. Reviews from other organizations can be cited as well!`
      ),
      highlights: "",
      decision: true,
      articleId: article.id,
  ...
  const venue = await prisma.venue.create({
    data: {
      ...
      abbreviation: "ACL",
      description: createEditorState("SubmissionTargetTypeahed"),
      organizationId: organization.id,
