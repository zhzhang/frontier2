...
const computeCompletionList = (cmdPrefix, commands, mostUsedCommands = {}) => {
  if (cmdPrefix === "") {
    return [];
  }
  const list = commands[cmdPrefix[0].toLowerCase()].filter((cmd) =>
    cmd.toLowerCase().startsWith(cmdPrefix.toLowerCase())
  );
...
function updateMostUsedTeXCmds(newTeX, mostUsedCommands, lastTex) {
  if (!mostUsedCommands) {
    return undefined;
  }
...
function mergeMacros(teXCmds, macros) {
  return Object.keys(macros).reduce(
    (red, m) => {
...
const initCompletion = (status, macros) => (editorState) => ({
  ...
  mostUsedTeXCommands:
    status !== "none" ? getInitialMostUsedTeXCmds(editorState) : undefined,
  getLastTeXCommand,
