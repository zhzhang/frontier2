import {
  SelectionState, ContentState, EditorState, Modifier,
} from 'draft-js';
...
export function removeEntity(contentState, blockKey, start, end) {
  const selToRemove = SelectionState.createEmpty(blockKey).merge({
    anchorOffset: start,
    focusOffset: end,
  });

  return Modifier.removeRange(contentState, selToRemove, 'backward');
}
